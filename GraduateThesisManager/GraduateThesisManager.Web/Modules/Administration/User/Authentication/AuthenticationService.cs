        ...
        private bool ValidateExistingUser(ref string username, string password, UserDefinition user)
        {
            ...
            if (user.IsActive != 1)
            {
                if (Log.IsInfoEnabled)
                    Log.Error($"Inactive user login attempt: {username}", this.GetType());

                return false;
            }

            // prevent more than 50 invalid login attempts in 30 minutes
            var throttler = new Throttler("ValidateUser:" + username.ToLowerInvariant(), TimeSpan.FromMinutes(30), 10);
            if (!throttler.Check())
                return false;
